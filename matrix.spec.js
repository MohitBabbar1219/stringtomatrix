...
describe('Matrix', () => {
  ...
  test('extract columns from one number matrix', () => {
    expect(new Matrix('1').columns[0]).toEqual([1]);
  });

  test('can extract columns', () => {
    expect(new Matrix('1 2 3\n4 5 6\n7 8 9').columns[2]).toEqual([3, 6, 9]);
  });

  test('can extract columns from non-square matrix', () => {
    expect(new Matrix('1 2 3\n4 5 6\n7 8 9\n8 7 6').columns[2]).toEqual([3, 6, 9, 6]);
  });

  test('extract columns where numbers have different widths', () => {
